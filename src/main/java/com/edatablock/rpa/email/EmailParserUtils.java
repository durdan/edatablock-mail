        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
             ...
             LOGGER.info("Start---------------------------------");
                ...
                LOGGER.info("From : "+mailMessage.getFrom()[0].toString());
                LOGGER.info("MessageId : "+mailMessage.getHeader("Message-ID")[0].toString());
                LOGGER.info("End---------------------------------");
            ...
            if (content instanceof String) {
                ...
                if (Part.ATTACHMENT.equalsIgnoreCase(disposition)) {
                    emailFragments.add(new EmailFragment(directory, i + "-" + filename, content,true));
                    LOGGER.info(String.format("Handdling attachment 2 '%s', type: '%s', hasattachment: '%s'", filename, contentType));
                }
                else {
                    ...
                    if(Part.ATTACHMENT.equalsIgnoreCase(disposition)){
                        LOGGER.info("Found a attachment hence setting a flag");
                        emailFragments.add(new EmailFragment(directory, textFilename, content,true));
                    }else
                ...
                if(Part.ATTACHMENT.equalsIgnoreCase(disposition)){
                    LOGGER.info("----Found an attachment hence setting a flag");
                    emailFragments.add(new EmailFragment(directory, filename, content,true));
                }else
